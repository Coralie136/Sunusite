...
class DocumentsControllerDocument extends JControllerForm
{
	...
	public function go()
	{
		...
		$modelLink	= $this->getModel('Document', '', array('ignore_request' => true));
		...
		if (empty($link))
		{
			return JError::raiseWarning(404, JText::_('COM_DOCUMENTS_ERROR_DOCUMENT_NOT_FOUND'));
		}
		...
		$modelCat = $this->getModel('Category', 'DocumentsModel', array('ignore_request' => true));
		...
		if ($link->url)
		{
			//$year = JHtml::_('date', $link->created, JText::_('Y'));
			$year = substr($link->created, 0, 4);
			$modelLink->hit($id);
			//JFactory::getApplication()->redirect($link->url);
			JFactory::getApplication()->redirect(JURI::base()."documents/".$link->url);
		}
		else
		{
			return JError::raiseWarning(404, JText::_('COM_DOCUMENTS_ERROR_DOCUMENT_URL_INVALID'));
		}
	}
}
