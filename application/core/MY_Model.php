...
class MY_Model extends CI_Model
{
    ...
    {
        parent::__construct();
        $this->load->database(); // Chargement de la bibliothèque de base de données
    }
	...
	//Modifie une ou plusieurs lignes dans la base de données.
	public function disable ($where, $options_echappees = array(), $options_non_echappees = array())
	{
		// Vérification des données à mettre à jour
		if(empty($options_echappees) AND empty($options_non_echappees))
		{
	...
	public function readOrder($select ='*', $where, $order) 
	{
		return    $this->db->select($select)
					->from($this->ma_table)
					->where($where)
...
//            $tOrWhere   .= '$this->db->or_where(\''.$key.'\', \''.$value.'\'); ';
//        }
...
//        }
//        
////        var_dump($joins['join0']['table']);/* exit;*/
////        var_dump($tJoin); /*exit;*/
////        var_dump($tWhere); /*exit*/;
////        var_dump($tOrWhere); /*exit*/;
//        
////        $euss   = '$this->db->select(\''.$select.'\')->from(\''.$this->ma_table.'\')'.$tJoin.$tWhere.$tOrWhere.$tOrder.'->limit('.$limit.')->get()->result();';
////        '$this->db->select(\''.$select.'\')->from(\''.$this->ma_table.'\')'.$tJoin.$tWhere.$tOrWhere.$tOrder.'->limit('.$limit.');';
////        var_dump($this->MY_Model); exit;
//        
...
//        eval($tOrder);
//		$this->db->limit($limit);
////        $result = $this->db->get($this->ma_table);
