...
class Article extends CI_Controller{
    ...
    public function __construct(){
        ...
        $this->load->model('article_table', 'article'); // load the article_table model as 'article'
        ...
        //-----------------------------------------------
            ...
            $this->data['page_description'] = 'gestion des articles';

            // get the class menu property
            $this->data['menu']             = $this->fonctions->menu;

            // set the active menu item
            $this->data['menu']['article']['article']   = 'active open';
    ...
    //-----------------------------------
        public function article() {
            ...
            //------------------------------------------
                ...
                $order      = array(
                    'article.dateCreated_article'   => 'DESC',
                );

                // execute the query and return all the required rows
                $this->data['articles'] = $this->article->readJoins('article.id_article, article.titre_article, article.texte_article, article.lecture_article, article.fichier_article, article.dateCreated_article, article.statut_article', $joins, $orWhere, $where, $limit, $order);
            ...
            $this->layer->pages('admin/article', 'admin/include/menu', $this->data);

        }
    //-----------------------------------
    
    //-----------------------------------------
    // DISPLAY AN EMPTY FORM TO ADD AN ARTICLE
    //-----------------------------------------
        public function newArticle () {
            ...
            // set the title of the current page
//            $this->data['page_description']             = 'Ajouter un article';
            ...
            // If the article's id is not empty
            if (!empty($aid)) { // article's data for update form

                // set the title of the current page
//                $this->data['page_description'] = 'Modifier l\'article';

                //----------------------------------
                // GET ALL THE ARTICLE INFORMATIONS
                //----------------------------------
                    // Execute the query and return all the required rows
                    $this->data['article']      = $this->article->read('*', array('id_article' => $aid));
    ...
    //----------------------------------------------
    // ADD (id == 0) OR UPDATE (id != 0) AN ARTICLE
    //----------------------------------------------
...
//                        var_dump(2147483648 + mt_rand( -2147482448, 2147483647 )); exit;
                        if (!empty($title) AND !empty($text)) {
                            ...
                            //-------------------------
                                ...
                                if (!is_dir('./upload/article')) {

                                    mkdir('./upload/article', 0777, true);
                            ...
                            if ($this->upload->do_upload('import-file')) {
                                ...
                                //--------------------------
                                    ...
                                    $inserted   = $this->article->create($toInsert);
                                    ...
                                    if ($inserted) {
                                        ...
                                        //----------------------------
                                            // execute the query and return all the required rows
                                            $insertedData   = $this->article->read('*', array('id_article' => $inserted));
                                        ...
                                        $retour = array('type' => 'success', 'message' => 'L\'article a &eacute;t&eacute; cr&eacute;&eacute; avec succ&egrave;s!');

                                    } else { // Return a warning message (the data already exists)
                                        $retour = array('type' => 'warning', 'message' => 'Une erreur est survenue, impossible de cr&eacute;er de l\'article');
                                    }
                                //-------------------------
...
//                                $retour = array('type' => 'warning', 'message' => '<strong>Erreur</strong>, impossible d\'enregistrer le fichier image associ&eacute;!');
                        ...
                        if (!empty($aid) AND !empty($title) AND !empty($text)) {
                            ...
                            if (!empty($files)) {
                                ...
                                if ($this->upload->do_upload('import-file')) { // upload file
                                    ...
                                    //-------------------------------
                                        // execute the query and return all the required rows
                                        $toUpdateData   = $this->article->read('*', array('id_article' => $aid));
                                    ...
                                    //--------------------------
                                        ...
                                        $updated    = $this->article->update($where, $toUpdate);
                                        ...
                                        if ($updated) {
                                            ...
                                            //--------------------------
                                                // execute the query and return all the required rows
                                                $updatedData    = $this->article->read('*', array('id_article' => $aid));
                                            ...
                                            $retour = array('type' => 'success', 'message' => 'L\'article a &eacute;t&eacute; modifi&eacute; avec succ&egrave;s!');
        ...
        public function enableArticle () {
            ...
            if (!empty($e_aid)) { // process
                ...
                //------------------------------------------------------------------------------
                    ...
                    // Stock the update status for test
                    $enabled    = $this->article->update($where, $toUpdate);
                //------------------------------------------------------------------------------
                ...
                if ($enabled) { // success, row's status updated
                    ...
                    //-------------------
                        ...
                        $toInsert   = array(
                            'user_log'  => $this->session->userdata('user_id'),
                            'table_log' => "article",
                            'data_log'  => "enable|statut_article|$aid|0|1",
                        );
                    ...
                    $retour = array('type' => 'success', 'message' => 'Article activ&eacute; avec succ&egrave;s!');

                } else { // warning, can't update row's status
                    // set the sendback, 'message' and 'type'
                    $retour = array('type' => 'warning', 'message' => 'Impossible d\'activer l\'article!');
                }
                ...
                foreach ($aids as $aid) {
                    if ($allDone) {
                        ...
                        //---------------------------------------------------------------------------
                            ...
                            // Stock the update status for test
                            $enable = $this->article->update($where, $toUpdate);
                        //---------------------------------------------------------------------------
        ...
        public function disableArticle () {
            ...
            if (!empty($d_aid)) { // process
                ...
                //------------------------------------------------------------------------------
                    ...
                    // Stock the update status for test
                    $disabled   = $this->article->update($where, $toUpdate);
                //------------------------------------------------------------------------------
                ...
                if ($disabled) { // success, row's status updated
                    ...
                    // set the sendback, 'message' and 'type'
                    $retour = array('type' => 'success', 'message' => 'Article d&eacute;sactiv&eacute; avec succ&egrave;s!');

                } else { // warning, can't update row's status
                    // set the sendback, 'message' and 'type'
                    $retour = array('type' => 'warning', 'message' => 'Impossible de d&eacute;sactiver l\'article!');
                }
        ...
        public function deleteArticle () {
            ...
            if (!empty($rm_aid)) { // process
                ...
                //------------------------------------------------------------------------------
                    ...
                    // Stock the update status for test
                    $deleted    = $this->article->update($where, $toUpdate);
                //------------------------------------------------------------------------------
                ...
                if ($deleted) { // success, row's status updated
                    ...
                    // set the sendback, 'message' and 'type'
                    $retour = array('type' => 'success', 'message' => 'Article supprim&eacute; avec succ&egrave;s!');

                } else { // warning, can't update row's status
                    // set the sendback, 'message' and 'type'
                    $retour = array('type' => 'warning', 'message' => 'Impossible de supprimer l\'article!');
                }
    ...
    //----------------------------------------------------------------------
    // GET ALL THE ARTICLE PICTURES AND DISPLAY EACH WITH OPTION TO ADD NEW
    //----------------------------------------------------------------------
        public function articlePhoto () {
            ...
            // Proccess only if there is a defined article
            if (!empty($aid)) {
...
//                        'global/plugins/bootstrap-fileinput/bootstrap-fileinput.css',
                ...
                // Get all the article photos only if there is a defined article
                if (!empty($this->data['article'])) {
                    
                    // Set the page description
                $this->data['page_description'] = ucfirst($this->data['article'][0]->titre_article);

                    //-------------------------------------------------
                    // Get list of all photos for the current article
                    //
                    // READ DATA FROM photo JOIN article ON id_article
                    //-------------------------------------------------
                        ...
                        // array of all the where condition using AND
                        $where      = array(
                            'article.id_article'        => $this->data['article'][0]->id_article,
                            'article.statut_article !=' => '3',
                ...
                // include the view with all the data requested
                $this->layer->pages('admin/article_photo', 'admin/include/menu', $this->data);
    ...
    //----------------------------------------
    // ADD PHOTOS TO THE CURRENT SENT ARTICLE
    //----------------------------------------
        public function addArticlePhoto () { // function to add photo to article
                
            // Get the defined article id
